...
from tatatele_whatsapp.api.whatsapp import send_whatsapp_msg

class SendNotification(Notification):
    def validate(self):
        self.validate_tatatele_whatsapp_settings()

    def validate_tatatele_whatsapp_settings(self):
        if self.enabled and self.channel == "WhatsApp" \
            and not frappe.db.get_value("Tatatele Whatsapp Settings",self.custom_whatsapp_account ,"url"):
